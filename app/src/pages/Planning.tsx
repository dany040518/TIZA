...
export default function Planning() {
  ...
  const handleUsePlan = async (idea: any) => {
    setActivePlan(idea);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6">
          ...
          <div className="h-full">
            <Card className="p-5 space-y-4 sticky top-4">
              <h2 className="text-lg font-semibold">Planifica tu clase</h2>
                ...
                <div>
                  {selectedIdea ? (
                    <div className="p-6 border rounded-2xl bg-card flex flex-col h-full">
                      <h2 className="font-semibold mb-2">{selectedIdea.title}</h2>
                        ...
                        {(selectedIdea.sequence || []).map((fase: any, i: number) => (
                          <div key={i} className="flex gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">{fase.duration} min</p>
                      ...
                      <Button className="mt-auto" onClick={() => handleUsePlan(selectedIdea)}>
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{activePlan.type}</p>
              </div>
              <div className="flex gap-2">
                <Button onClick={handleDownloadPDF} variant="outline">PDF</Button>
                <Button variant="ghost" onClick={() => setActivePlan(null)}>✖</Button>
              </div>
            </div>
            ...
            <div className="space-y-5">
              {(activePlan.sequence || []).map((fase: any, i: number) => (
                <div key={i} className="flex gap-3">
                  ...
                  <div>
                    <p className="font-medium text-sm">{fase.phase} ({fase.duration ?? "-"} min)</p>
                    <p className="text-sm text-muted-foreground">{fase.description}</p>
                  </div>
                </div>
              ))}
            ...
            <div className="mt-6">
              ...
              <p className="text-sm text-muted-foreground">{activePlan.evaluation}</p>
            </div>
          </div>
        )}
